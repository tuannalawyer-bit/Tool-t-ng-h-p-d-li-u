...
def trace_clkk():
    ...
    with zipfile.ZipFile(filepath, 'r') as z:
        ...
        # Let's read the first 100 rows and print ALL cells to see structural alignment
        print("=== TRACING CLKK_20% DATA STRUCTURE ===")
                ...
                if event == 'start':
                    ...
                    if tag.endswith('}c'):
                        ...
                        for v in row_cells.values():
                            ...
                            if 'Cảnh báo:' in sval:
                                has_warning = True
                            if sval.startswith('WM+') or sval.startswith('WIN') or sval.startswith('[Block]'):
                                has_store = True
                        
                        if has_warning and not has_store:
                            print(f"ROW {current_row_idx} (Has warning but NO STORE KEYWORD):")
