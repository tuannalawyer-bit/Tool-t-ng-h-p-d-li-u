...
def process_msg_file(msg_path, log_callback=None):
    ...
    try:
        ...
        if not truong_nhom:
            # Mô phỏng AI Gemini: Tìm Trưởng nhóm trong danh sách To / Cc
            known_tns = [
                'Lê Khánh Hà', 'Trần Huy Dũng', 'Nguyễn Xuân Ngọc', 'Vương Lê Duy', 
                'Ngô Hữu Hoàn', 'Nguyễn Đăng Trường', 'Trương Thanh Tùng', 
                'Nguyễn Minh Lợi', 'Tạ Thị Tình', 'Phạm Văn Thái'
            ]
            mail_headers = str(msg.to or "") + " " + str(msg.cc or "")
            for tn in known_tns:
                if tn.lower() in mail_headers.lower():
                    truong_nhom = tn
                    break
