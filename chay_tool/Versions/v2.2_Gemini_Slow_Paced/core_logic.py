...
GEMINI_PROMPT_TEXT = """Bạn là chuyên gia trích xuất dữ liệu cao cấp. Nhiệm vụ của bạn là đọc toàn bộ email (bao gồm Subject, From, To, Cc và Body) để trích xuất và TÌM KIẾM CÁC TRƯỜNG BỊ THIẾU.
Tuyệt đối KHÔNG BỊA DỮ LIỆU. Nếu không có cơ sở chắc chắn, hãy trả về chuỗi rỗng "".

🔍 MẸO TÌM KIẾM DỮ LIỆU ẨN (Suy luận Logic):
1. Tỉnh/Thành phố: Nếu trong bảng không ghi, hãy tự suy luận từ Địa chỉ của mặt bằng, hoặc từ 3 chữ cái trong Mã MB (VD: HNI -> Hà Nội, BNH -> Bắc Ninh, SGN -> TP.HCM, HPG -> Hải Phòng).
2. TN TKMB (Trưởng nhóm): Nếu bảng trống, hãy tìm tên người trong danh sách To/Cc, hoặc người được tag (@name) trong body.
3. Mã MB: Nếu bảng trống, tìm mã trong Tiêu đề (Subject) (VD: MB07930, 2B65, MN123...).
4. BCT: Tìm trong đoạn text có các từ "Cô/Chú/Anh/Chị/Ông/Bà" kèm theo Số điện thoại (VD: Chú Tuấn 090xxxx).
5. Diện tích (DTT): Tìm trong text các đoạn "thuê ... m2" hoặc "diện tích ... m2".

Trả về KẾT QUẢ DUY NHẤT LÀ MỘT CHUỖI JSON HỢP LỆ (không có markdown, không giải thích).
Keys cần có:
- ma_mb: Mã mặt bằng (ví dụ: MB07930, MN2817, 2B65... KHÔNG BAO GIỜ lấy các từ tiêu đề như "Thông tin", "Giá thuê")
- tinh_trang: Tình trạng (Mở mới/Tái ký/Đang thuê)
- thong_tin_mb: Tên mặt bằng (hoặc địa chỉ)
- tinh: Tỉnh/Thành phố
...
def call_gemini_text(raw_text, retries=3, log_callback=None):
    """Gửi nội dung text email lên Gemini để trích xuất dữ liệu, có xử lý Rate Limit."""
    if not GEMINI_AVAILABLE or not _gemini_client: return {}
    
    for attempt in range(retries):
        try:
            if log_callback: log_callback("⏳ Đang đợi 5s để tránh Rate Limit (Text)...", "info")
            time.sleep(5) # Nhịp độ chậm lại: 5s / request
            ...
            if '429' in err_msg or 'Quota' in err_msg or 'RESOURCE_EXHAUSTED' in err_msg:
                try:
                    # Fallback model nếu Pro hết hạn ngạch ngày
                    response = _gemini_client.models.generate_content(
                    ...
                    if attempt < retries - 1:
                        print(f"⏳ Rate limit! Đang chờ 15s để thử lại (lần {attempt+1}/{retries})...")
                        time.sleep(15)
                        continue
                    else:
                        print(f"❌ Gemini Flash fallback error: {e2}")
                        return {}
            else:
...
def call_gemini_image(image_path, retries=3, log_callback=None):
    """Gửi ảnh lên Gemini Vision để trích xuất dữ liệu, có xử lý Rate Limit."""
    if not GEMINI_AVAILABLE or not _gemini_client: return {}
    
    for attempt in range(retries):
        try:
            if log_callback: log_callback("⏳ Đang đợi 5s để tránh Rate Limit (Image)...", "info")
            time.sleep(5) # Nhịp độ chậm lại: 5s / request
            ...
            if '429' in err_msg or 'Quota' in err_msg or 'RESOURCE_EXHAUSTED' in err_msg:
                try:
                    ...
                    if attempt < retries - 1:
                        print(f"⏳ Rate limit (Image)! Đang chờ 15s để thử lại (lần {attempt+1}/{retries})...")
                        time.sleep(15)
                        continue
                    else:
                        print(f"❌ Gemini Vision Flash fallback error: {e2}")
                        return {}
            else:
...
def process_images_with_gemini(msg, msg_path, log_callback=None):
    ...
    for a in msg.attachments:
        ...
        if fname.lower().endswith(('.png', '.jpg', '.jpeg')) and a.data:
            ...
            try:
                ...
                data = call_gemini_image(temp, log_callback=log_callback)
...
def process_msg_file(msg_path, log_callback=None):
    ...
    try:
        ...
        # Validate TN TKMB: phải là tên người, không phải giá thuê/SĐT
        if truong_nhom and re.search(r'VND|tháng|\d{7,}|\(con gái', truong_nhom, re.IGNORECASE):
            truong_nhom = ''

        # Validate Mã MB: Không được là các header bị nhận nhầm
        if ma_mb:
            bad_kws = ['thông tin', 'giá thuê', 'diện tích', 'dtt', 'tình trạng', 'bct', 'stt']
            if any(kw in ma_mb.lower() for kw in bad_kws):
                ma_mb = ''  # Parser nhầm header thành giá trị

        # Validate DTT: phải là số, không phải SĐT
        if dtt and re.search(r'[()]|^0\d{8,}', str(dtt)): dtt = ''

        # Validate BCT: không thể chỉ là con số giá tiền
        if bct and re.match(r'^[\d.,\s]+$', bct.strip()):
            bct = ''
        ...
        # Kiểm tra dữ liệu đáng nghi (Regex parse sai hoặc lệch cột)
        suspicious = False
        
        # Nếu Mã MB hoặc DTT hoặc Giá thuê bị sai/lệch cột
        if not ma_mb or not dtt or not gia_thue: suspicious = True
        # Nếu BCT quá dài (thường do gộp nhầm cột TN)
        if len(str(bct)) > 30: suspicious = True
        
        need_ai = bool(missing) or suspicious

        if need_ai and GEMINI_AVAILABLE and _gemini_client:
            reason = f"Thiếu {len(missing)} trường" if missing else "Dữ liệu lệch cột/đáng nghi"
            if log_callback: log_callback(f"🤖 Bước 2: AI Rescue - {reason}", "warn")

            # 2a: Gemini Vision cho ảnh đính kèm
            img_data = process_images_with_gemini(msg, msg_path, log_callback=log_callback)

            # 2b: Gemini Text cho nội dung email (Truyền toàn bộ metadata để AI làm thám tử)
            full_context = (
                f"Subject: {subject}\n"
                f"From: {msg.sender}\n"
                f"To: {msg.to}\n"
            ...
            text_data = call_gemini_text(full_context[:8000], log_callback=log_callback)
            ...
            # Merge: Gemini LUÔN ƯU TIÊN GHI ĐÈ dữ liệu từ Regex nếu có kết quả
