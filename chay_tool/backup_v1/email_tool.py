...
def write_to_excel(results, excel_path, log_callback=None):
    ...
    COL_MAP = [
        ...
        (11, "bct"), (12, "cv_tkmb"), (13, "tn_tkmb"),
        (14, "chi_tiet_thamdinh"), (15, "gia_thue_sum"),
        (16, "hspl"), (17, "ghi_chu"),
    ]
    ...
    for i, r in enumerate(results_to_write):
        ...
        ws.cell(row=row_num, column=13, value=d.get("tn_tkmb", ""))
        ws.cell(row=row_num, column=14, value=d.get("chi_tiet_thamdinh", ""))
        ws.cell(row=row_num, column=15, value=d.get("gia_thue_sum", ""))
        ws.cell(row=row_num, column=16, value=d.get("hspl", ""))
        ws.cell(row=row_num, column=17, value=d.get("ghi_chu", ""))

        # Highlight missing fields in yellow
        FIELD_TO_COL = {
            'thang': 2, 'ngay_nhan': 3, 'ngay_tra': 4, 'ma_mb': 5,
            'tinh_trang': 6, 'thong_tin_mb': 7, 'tinh': 8, 'dtt': 9,
            'gia_thue_vat': 10, 'bct': 11, 'cv_tkmb': 12, 'tn_tkmb': 13,
            'chi_tiet_thamdinh': 14, 'gia_thue_sum': 15, 'hspl': 16,
        }
...
class App(tk.Tk):
    ...
    def _build_ui(self):
        ...
        fl_hdr = tk.Frame(left, bg=BG_CARD, pady=8, padx=10)
        fl_hdr.pack(fill="x", pady=(0, 4))
        tk.Label(fl_hdr, text="📁 File .msg trong thư mục", font=("Segoe UI", 10, "bold"),
    ...
    def _scan_files(self):
        ...
        pattern = os.path.join(FOLDER, "*.msg")
    ...
    def _run_processing(self, files, excel_path):
        ...
        for r in results:
            if not r["success"]:
                ...
                # Có dữ liệu nhưng không đầy đủ → failed_
                rename_file(r["path"], "failed")
            else:
                # Đầy đủ 100% → done_
