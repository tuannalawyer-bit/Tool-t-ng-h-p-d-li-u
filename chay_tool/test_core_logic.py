...
def process_msg_file(msg_path, log_callback=None):
    ...
    try:
        ...
        if missing and msg.attachments:
            ...
            for a in msg.attachments:
                ...
                if fname.lower().endswith(('.png', '.jpg', '.jpeg')) and a.data:
                    temp = os.path.join(os.path.dirname(os.path.abspath(msg_path)), "_temp_ocr_" + fname)
                    try:
                        with open(temp, 'wb') as f: f.write(a.data)
                        reader = get_ocr()
                        res = reader.readtext(temp, detail=0)
                        ocr_text += " ".join(res) + " \n"
                    except: pass
                    finally:
                        try: os.remove(temp)
        ...
        # Cleanups
        if truong_nhom and re.search(r'VND|tháng|\d{7,}|\(con gái', truong_nhom, re.IGNORECASE): truong_nhom = ''
        ...
        try:
            ...
            if kstt_idx != -1: kstt_text = body[kstt_idx:kstt_idx+1000].strip()
        except: pass
